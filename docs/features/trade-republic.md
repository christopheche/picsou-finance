...
Both session writes made on the way out of a failed sync — the **delete** after a rejected refresh, and the **rotated tokens** of a refresh that succeeded before the retry failed transiently — go through `TradeRepublicSessionWriter`, a separate bean whose methods are `@Transactional(REQUIRES_NEW)`. The service is class-level `@Transactional` and rethrows `SyncException` to the controller, which marks its transaction rollback-only: a plain `sessionRepository.delete` was rolled back with it, so the row survived, `getSessionStatus` kept answering `active=true`, and every click on Sync repeated the sidecar round-trip and the same 422 instead of offering the re-auth form; likewise a WebSocket timeout on the retry restored the *old* refresh token TR had just rotated away, and the next sync cleared the session over a transient failure. Only the scheduled path, which catches inside the transaction, ever committed them. The service no longer mutates the managed session entity, which is what keeps the `REQUIRES_NEW` writes free of the row-lock hang documented on `CryptoExchangeStatusWriter`. Same shape as `IbkrStatusWriter` / `DegiroSessionStatusWriter`; `TradeRepublicSessionWriterTest` (H2 slice) proves both writes survive the caller's rollback.
...
`TradeRepublicSyncService.importCsv()` parses a CSV file with columns `name,type,balance`. Accounts are deduplicated via a stable external ID derived from the name (`tr_csv_` prefix + slugified name). Only an `IOException` on the upload is wrapped (`SyncException("Could not read the CSV file…", cause)`); anything `upsertAccount` throws — a missing member, a constraint violation — propagates with its own status. The old `catch (Exception)` relabelled those as a parse error, concatenated `ex.getMessage()` into the 422 detail the user sees (SQL included) and dropped the cause. Malformed lines, non-numeric balances and unknown types are skipped / mapped to `OTHER` with a WARN, as pinned by the `importCsv_*` tests.
...
| Input validation with `@NotBlank` + `@Size(max = 100)` on the DTOs, `@Valid` on the controller methods | Prevents null phone/PIN from reaching the `Map.of()` constructor in `TradeRepublicAdapter`, which rejects nulls and throws `NullPointerException` — a 500 where the caller should get a 422 naming the field | Try-catch in adapter (less clear, reactive) |
...
- **Input validation is strict**: `InitiateAuthRequest` and `CompleteAuthRequest` enforce `@NotBlank @Size(max = 100)` on all fields and both controller methods carry `@Valid`. Empty or null values result in a 422 response before reaching the service layer. Ensure frontend sends valid non-blank values.
- **Every entry point is throttled, on three separate budgets**: `/auth/initiate` keeps its strict SMS budget (`trAuthBuckets`, 3 per 10 min — each attempt sends an SMS); `/auth/complete` has its own (`trTanBuckets`, 5 per 15 min) because the TAN space is only 10,000 and an unthrottled verify endpoint is brute-forceable once a `processId` is known — sharing the SMS budget would instead lock a member out of their own login after one mistyped code; `/sync` and `/import` share the per-IP `syncBuckets` (10 per minute) like every other sync and upload entry point, since each sync decrypts the stored session and drives a sidecar round-trip, and the 10 MB multipart ceiling is global. Past a budget the controller answers 429 with a ProblemDetail.
...
- **timeout-driven completion**: The WebSocket session completes when either all data is received (cash + all portfolios + all tickers) or a 30-second idle timeout is hit.
- **An unreadable cash payload is never a 0 EUR balance**: `parseCashValue` returns `Optional<BigDecimal>`, and `parseCashJson` emits **no** `TR Cash` account when it is empty — a blank body, an error frame, or a shape carrying neither `value` nor `amount`. Returning `BigDecimal.ZERO` for all of those made "the account holds nothing" and "we could not read the answer" the same number, which `TradeRepublicSyncService.upsertAccount` wrote as `currentBalance` **and** stamped as the day's `BalanceSnapshot`; `upsertSnapshot` overwrites an existing same-day row, so one bad frame put a permanent hole in the net-worth chart. Skipping leaves the previous balance standing — the same refusal `CryptoExchangeSyncService` and `WalletSyncService` make.
- **The frame type is read, not just the payload**: a TR frame is `<id> <type> <payload>` (`A` answer, `C`/`D` delta, `E` error). `extractWsPayload` drops the type token, so an `E` frame for the cash or portfolio subscription used to be stored as if it were an answer and parsed to 0. `extractWsType` is now consulted: an `E` frame on the default cash sub emits no `TR Cash` account, on a scoped (PEA) cash sub skips that account, and on a portfolio sub leaves the account out entirely rather than valuing it at 0. `E` frames on **ticker** subs keep their existing tolerance — they count as answered and the position falls back to `averageBuyIn`, which is documented behaviour (issue #23). Only the first frame per cash subscription is read, for the same reason `answeredTickerSubs` reads only the first ticker frame.
- **A PEA without its cash pocket is skipped, not understated**: the PEA balance is seeded from its scoped `availableCash(accountNumber=…)` frame. When that frame was expected (the account has a `cashAccountNumber`) and no value came back — never arrived before the idle timeout, rejected, or unparseable — the account is left out of the returned list instead of being written at positions-only value, which would silently drop the cash from net worth and snapshot it.
...
- **The background sync is handed over only after `completeAuth` commits**: `submitAfterCommit` registers the `runAsync` in a `TransactionSynchronization.afterCommit` (inline when no transaction is active, i.e. unit tests). Submitting inline from inside the still-open transaction let the tr-sync thread run `findById` before the session row was committed: a `null` session, which the expiry path read as "no refresh token" and answered by deleting whatever session `findByMemberId` found — the one just stored — telling the user to reconnect right after connecting. Its catch-all also logs a `SyncException` at WARN and anything else at ERROR **with the trace** (it used to log `ex.getMessage()` alone, which is `null` for an NPE, on the one run of the initial sync the user never sees).
...
- `TradeRepublicAdapterTest` -- `refreshSession` status mapping; the `availableCash` parsers (both payload shapes, and every unreadable one reporting *nothing* rather than zero); `extractWsType`; JWT sec-account extraction pairing each securities account with its own cash account; and `fetchAccounts` driven end to end against a local WebSocket server (positions priced from tickers, the PEA cash pocket added, the `averageBuyIn` fallback on a rejected ticker sub, no `TR Cash` on a rejected cash sub, the PEA skipped when its cash pocket does not arrive, and `SESSION_EXPIRED` on an auth error frame)
...
- `TradeRepublicSyncServiceTest` (session lifecycle) -- rejected refresh and rejected retry clear the session through `TradeRepublicSessionWriter`, a transient refresh failure keeps it, rotated tokens are stored through the writer and survive a failed retry, and `completeAuth` hands the background sync over only in `afterCommit` (inline without a transaction)
- `TradeRepublicSessionWriterTest` -- `@DataJpaTest`/H2 slice: `clear` and `storeRefreshedTokens` commit even when the calling transaction rolls back, and an un-rotated refresh token is kept
- `TradeRepublicSyncServiceTest#importCsv_*` -- header + rows parsed into upserts with the stable `tr_csv_<slug>` external id, malformed / non-numeric rows skipped, unknown type → `OTHER`, the same name twice updating one account, an `IOException` wrapped with its cause and without its message, a business failure propagating unchanged
