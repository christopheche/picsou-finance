...
- **A 429 pauses every Yahoo price call, not just the one that hit it**: Yahoo is fetched **per ticker** (it has no batch endpoint for EUR conversion), so a portfolio that trips the limit at ticker 20 used to still fire the remaining 40 requests — plus one FX request each — and the 15-minute scheduler repeated that every cycle. `YahooFinancePriceProvider` now records `rateLimitedUntil` on a 429 (`Retry-After` when it is sane, else 60 s, capped at 15 min), abandons the rest of that batch and sends nothing at all while the pause holds. Same mechanism, same reasoning as `CoinGeckoPriceProvider`. A **404** is one symbol's problem, not the endpoint's, and deliberately does *not* stop the batch.
- **Yahoo: expected outages return no price; genuine bugs propagate**: `getPricesEur` used to catch `Exception` per ticker and log `ex.getMessage()` alone, so a parser defect against a changed Yahoo payload was indistinguishable from an outage and read as `"…failed for AAPL: null"`. It now grades the failure like `CoinGeckoPriceProvider.handleFetchFailure` — **WARN** for an HTTP status, a timeout (with its duration) or an unreachable API, and everything else **rethrown** so it surfaces at ERROR with a trace in the guarded batch loops. Unwrapping first is mandatory: `Mono.timeout()` signals a *checked* `TimeoutException` that `block()` wraps in a reactor `ReactiveException`. Rethrowing is only safe because `SchedulerService.dailySnapshots`/`refreshPrices` and `PriceService.backfillHistoricalPrices` guard their own loops — see the CoinGecko entry in [crypto-tracking.md](./crypto-tracking.md).
...
- `YahooFinancePriceProviderTest` -- unit tests for response parsing, plus failure grading: a 429 stops the batch after one request and keeps the provider silent on the next call, a 404 does not stop the batch, and a non-upstream `RuntimeException` propagates instead of being swallowed as a missing price
