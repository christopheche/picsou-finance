...
`/institutions` is throttled too, on its own `ip:institutions` key but with a **typeahead-sized** budget (60/min, `RateLimitConfig.createInstitutionSearchBucket`) rather than the 10/min sync one: unlike `/countries` it is served from no cache at all — every call is an authenticated fetch from the provider, and Germany alone is ~1.4 MB across ~1100 institutions — but the search box fires a request per keystroke with no debounce, so a 10/min budget would 429 someone typing a bank name. Same shape and reasoning as the address autocomplete limiter.
...
- **A missing balance is never a zero**: `AccountData.balance` is `null` when Enable Banking returned no balance for an account — an empty `balances` list (common while a freshly-linked account is still propagating, or when consent covered details but not balances) or items carrying no `balance_amount`. `upsertAccount` then keeps the account's last known `currentBalance`, writes **no** snapshot for the day, and still refreshes `lastSyncedAt`/logo/requisition link; a brand-new account is not created at all until a real balance arrives. Persisted as `0.00` this used to overwrite a real balance and stamp a zero into that day's `balance_snapshot` — and `upsertSnapshot` overwrites an existing same-day row, so the dip stayed in the net-worth chart after the next good sync fixed the balance. Same refusal `WalletSyncService`, `CryptoExchangeSyncService` and `SchedulerService.dailySnapshots` already make. Note `markRetryableIfEmpty` does not cover this: it inspects the account *list*, never a per-account balance.
- **Powens maps no product hint, and its balances are `BigDecimal`**: `PowensBankConnector.toAccountData` used to compute a product string "that `SyncService.detectType()` understands" and then drop it — no such method exists (see the *Known gap* above) and `AccountData` has no product field, so the mapping was dead code that read as a working feature. It is gone; re-adding PEA/LEP detection means adding a hint to `AccountData` **and** a consumer in `SyncService`. The Powens balance is also deserialised straight into `BigDecimal` rather than `Double` + `BigDecimal.valueOf`, so money never passes through binary floating point.
...
- **The scheduled resync commits one requisition at a time**: `SyncService.resyncAll` and `retryAllFailed` opt out of the class-level `@Transactional` (`propagation = NOT_SUPPORTED`) and run each requisition's writes in their own `TransactionTemplate` transaction, with the Enable Banking round-trip made *before* it opens. Catching the exception alone was not isolation: a repository call throwing for one bank (a duplicate `account` row, the `balance_snapshot (account_id, date)` constraint) marked the member-wide transaction rollback-only through the repository's proxy, the catch saved FAILED and logged, and the commit at method exit threw `UnexpectedRollbackException` — discarding every other bank's balances, snapshots and `lastSyncedAt` together with the FAILED mark, and leaving `SchedulerService` to log one generic "Daily Enable Banking sync failed" for the member. The FAILED mark now goes through `RequisitionLifecycleWriter.markFailed` (`REQUIRES_NEW`), so it survives whatever the failed transaction did. The two scheduled loops also split their catch like every other auto-sync entry point: a `SyncException` (dead session, provider down) is one WARN line, anything else is ERROR with the trace. `retryAllFailed` self-invokes `retrySync`, so that method's own `@Transactional(noRollbackFor)` never applies on the scheduled path — the template is its only boundary there.
...
- `SyncServiceTest` -- unit tests for type detection, upsert logic (including a `null` balance keeping the last known one, writing no snapshot and creating no account), retry flow, checkpoint ordering on fetch/upsert failures, logo matching for both the current and the legacy institution id format, and the scheduled loops' isolation (one requisition's persistence failure rolls back alone, is marked FAILED through the lifecycle writer and logged at ERROR with a trace; a provider failure is one WARN and opens no transaction; a still-dead retry does not stop the next)
- `SchedulerServiceTest` -- a throwing connector (including a proxy-exit `UnexpectedRollbackException` from Trade Republic or IBKR) does not stop the other connectors of that member nor the next member
...
- `SyncControllerTest` -- `GET /countries` returns the service result; rate-limit-exceeded returns 429 without calling the service; `/countries` and `/initiate` use independent buckets; `GET /institutions` returns the service result, answers 429 on its own exhausted `ip:institutions` bucket without calling the provider, and tolerates a 30-request typeahead burst
